...
interface DateTimePicker24hProps {
  value?: Date; // New value prop
  onChange?: (date: Date | undefined) => void; // onChange prop
}
...
export function DateTimePicker24h({ value, onChange }: DateTimePicker24hProps) {
  ...
  const handleDateSelect = (selectedDate: Date | undefined) => {
    if (selectedDate) {
      if (onChange) onChange(selectedDate); // Call onChange when date is selected
    }
  };
  ...
  const handleTimeChange = (type: "hour" | "minute", newValue: string) => {
    if (value) {
      ...
      if (onChange) onChange(newDate); // Call onChange when time is changed
    }
  };
